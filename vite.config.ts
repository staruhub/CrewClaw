...
const crewclawRootCommand =
  process.env.VITE_CREWCLAW_ROOT_COMMAND ??
  `pnpm --silent -C ${quoteShellArgument(__dirname)} run crewclaw`;
...
export default defineConfig({
  ...
  define: {
    "import.meta.env.VITE_CREWCLAW_ROOT_COMMAND":
      JSON.stringify(crewclawRootCommand),
  },
  server: {
    port: 3000,
  },
