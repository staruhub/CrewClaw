...
import htm from "htm";
import { Header, UserMessage, AssistantMessage, ToolLine } from "./components.mjs";

const html = htm.bind(React.createElement);

// stub renderer until codex's ui-markdown.renderMessage lands (DI keeps this testable now)
const stubRender = (text) => String(text).split("\n").map((l) => "   " + l);

const committed = [
  { role: "user", text: "查一下最近的大模型发布" },
  { role: "assistant", text: "我先搜一下最近一周的官方动态。\n\n找到三条:\n- A 模型\n- B 模型\n- C 模型" },
];
...
function Demo() {
  ...
  React.useEffect(() => {
    const toks = ["根据", "刚才", "的", "搜索", "，", "本周", "共", "三家", "发布", "了", "新", "模型", "……"];
    let i = 0;
    const tn = setTimeout(() => setTools([{ text: "⌕ web_search「最近发布」(3 处)", status: "success" }]), 180);
    const t = setInterval(() => {
      if (i < toks.length) setLive((s) => s + toks[i++]);
      else { clearInterval(t); setTimeout(() => done && done(), 100); }
    }, 30);
    return () => { clearInterval(t); clearTimeout(tn); };
  }, []);
  return html`
    <${Box} flexDirection="column">
      ...
      <${Static} items=${committed}>
        ${(m, i) => m.role === "user"
          ...
          : html`<${AssistantMessage} key=${i} name="鲸" lines=${stubRender(m.text)} />`}
      </>
