...
export function createWorkbenchStore(meta = {}, initialTurns = []) {
  ...
  const commit = errored => {
    ...
    currentRun = null;
    set({
      turns: [...state.turns, { role: "assistant", app, errored: !!errored }],
      live: null,
      sessionUsage,
      status: errored ? "error" : "idle",
      sessionPendingActions: app.pendingActions || [],
    });
  };
  ...
  return {
    ...
    startTurn(info = {}) {
      const run = createTaskRun(meta, app =>
        set({ live: app, status: app.status })
      );
      currentRun = run;
    ...
    failTurn(reason) {
      if (currentRun) currentRun.reject(reason);
      commit(true);
    },
