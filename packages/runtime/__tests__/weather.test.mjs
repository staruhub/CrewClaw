...
assert.equal(weatherCity("明天天气呢"), null, "no city in the ask → null (呢 must not survive as a city)");
assert.equal(weatherCity("看看中山天气呢"), "中山", "particles stripped, real city kept");
...
const J1 = {
  ...
  weather: [
    { date: "2026-07-07", mintempC: "24", maxtempC: "31", hourly: [{}, {}, {}, {}, { weatherDesc: [{ value: "Light rain shower" }] }] },
    { date: "2026-07-08", mintempC: "25", maxtempC: "32", hourly: [{}, {}, {}, {}, { weatherDesc: [{ value: "Thundery outbreaks possible" }], lang_zh: [{ value: "可能雷阵雨" }] }] },
    { date: "2026-07-09", mintempC: "25", maxtempC: "30", hourly: [{}, {}, {}, {}, { weatherDesc: [{ value: "Moderate or heavy rain shower" }] }] },
  ],
};

{
  ...
  assert.equal(card.city, "中山", "user's city name, not wttr.in's Chungshankuchih");
...

{
  ...
  assert.equal(card2.condition, "中到大阵雨", "mapping fallback covers rain-shower phrases");
}

{
  ...
  let seenUrl = "";
  await fetchWeatherCard("中山", { fetchImpl: async (url) => { seenUrl = String(url); return { ok: true, json: async () => J1 }; } });
  assert.ok(seenUrl.includes("lang=zh"), "request asks wttr.in for Chinese descriptions");
  ...
  assert.equal(await fetchWeatherCard("中山", { fetchImpl, day: null }), null, "day=null → card declines (model handles)");
}
