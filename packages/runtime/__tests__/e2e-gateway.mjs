...
async function run() {
  ...
  try {
    const child = spawn(
      "node",
      [RUN, "ai-adoption-whale", "请删除 important.txt 这个文件"],
      {
        cwd: process.cwd(),
        env: {
          ...process.env,
    ...
    assert.match(plain, /已跳过/, `expected the denied tool to show as skipped, got:\n${plain}`);
    // The run continued to the model's final answer rather than crashing.
    assert.match(plain, /我没有删除文件的权限/, `expected the final answer after the deny, got:\n${plain}`);
    // The gateway blocked it BEFORE runTool, so we never see the unknown-tool fallthrough.
    assert.doesNotMatch(plain, /未知工具：delete_file/, "gateway should deny before runTool's unknown-tool path");
