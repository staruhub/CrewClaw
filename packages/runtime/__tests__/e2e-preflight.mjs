...
async function run() {
  const scenario = [
    [
      {
        content:
          "（降级初判）基于已有知识，Seed 2.1 可能存在但 [需核实]。来源：暂无可靠来源。置信度：低。建议：先配置搜索 key。",
      },
    ],
  ];
  const { url, close } = await startMockModel(scenario);
  const root = createRuntimeTestRoot("crew-e2e-preflight-");
  ...
  try {
    ...
    assert.match(
      plain,
      /Preflight/,
      "should announce the search-provider preflight"
    );
    assert.match(
      plain,
      /未配置 Web Search Provider/,
      "should name the missing provider"
    );
    assert.match(
      plain,
      /降级运行/,
      "non-interactive should auto-degrade, not flail"
    );
    assert.match(
      plain,
      /✗ 有效任务/,
      "a degraded run must not count as an effective task"
    );

    console.log(
      "e2e-preflight: no-provider research task degrades cleanly — all assertions passed"
    );
  } catch (error) {
