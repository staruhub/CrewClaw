...
async function run() {
  ...
  const root = createRuntimeTestRoot("crew-e2e-task-");
  ...
  try {
    ...
    assert.match(
      plain,
      /研究计划/,
      "should show the Search Planner before working"
    );
    assert.match(
      plain,
      /site:volcengine\.com/,
      "the plan should include an official-domain query"
    );
    assert.match(
      plain,
      /官方名称|Doubao-Seed-2\.1/,
      "the deliverable should be rendered live"
    );
    assert.match(plain, /任务验收/, "should print the acceptance panel");
    assert.match(
      plain,
      /✓ 结构达标/,
      `required sections should validate, got:\n${plain}`
    );
    assert.match(
      plain,
      /✓ 验收规则/,
      `rubric rule-check should pass, got:\n${plain}`
    );
    assert.match(
      plain,
      /状态 delivered/,
      "should reach delivered (no user signal when piped)"
    );
    assert.match(
      plain,
      /反馈：skipped/,
      "effective feedback skipped when stdin is not a TTY"
    );
    assert.match(
      plain,
      /TaskRun → \.crewclaw\/runs\/task_/,
      "should persist a TaskRun record"
    );
    assert.match(
      plain,
      /Cost: \$/,
      "should show the task token cost (Budget Guard)"
    );
    assert.doesNotMatch(
      plain,
      /📓 沉淀 \d+ 条记忆/,
      "a delivered task without user acceptance must not claim saved memory"
    );
    assert.deepEqual(
      loadMemory(root, "ai-adoption-whale").items,
      [],
      "a delivered task without user acceptance must not write searchable memory"
    );
    assert.match(
      plain,
      /🔖 \d+ 条证据卡/,
      "should bind cited sources to evidence cards (Evidence Store)"
    );
    assert.match(
      plain,
      /员工动作/,
      "should show the action recap (event summaries)"
    );
    assert.match(
      plain,
      /正在执行命令/,
    ...
    console.log(
      "e2e-task: --task closed loop (resolve → gate → grade → record) — all assertions passed"
    );
  } catch (error) {
