...
async function run() {
        ...
        tool_calls: [
          {
            ...
            function: {
              ...
              arguments: JSON.stringify({
                url: "https://duckduckgo.com/html/?q=Seed+2.1+volcengine",
              }),
            },
          },
        ],
  ...
  const root = createRuntimeTestRoot("crew-e2e-serp-");

  try {
    const child = spawn(
      process.execPath,
      [RUNTIME_ENTRY, "ai-adoption-whale", "查一下 Seed 2.1"],
      {
        cwd: REPO_ROOT,
        env: {
          ...process.env,
    ...
    assert.match(
      plain,
      /duckduckgo\.com.*已跳过/,
      `the SERP fetch should render as refused, got:\n${plain}`
    );
    assert.match(
      plain,
      /改用 web_search/,
      "the agent should continue past the block, not re-flail"
    );

    console.log(
      "e2e-serp: SERP scrape blocked at the tool level — all assertions passed"
    );
  } catch (error) {
