...
function makeRoot({ approval, accepted } = {}) {
  ...
  if (approval !== undefined) {
    ...
    fs.writeFileSync(path.join(root, ".crewclaw", "prefs.json"), JSON.stringify({ approval }));
  }
  if (accepted !== undefined) {
    fs.mkdirSync(path.join(root, ".crewclaw", "kpi"), { recursive: true });
    fs.writeFileSync(
      path.join(root, ".crewclaw", "kpi", "policy-agent.json"),
      JSON.stringify({ tasks: accepted, accepted, total_cost: 0, first_hired_ts: 1 }),
    );
  }
  return root;
}
...
async function defaultPolicyHoldsForManualApproval() {
  ...
  assert.ok(types.includes("approval.requested"), "default policy holds the deliverable for manual accept");
  assert.ok(!types.includes("task.completed"), "held deliverable does not auto-complete");
  assert.ok(!types.includes("approval.accepted"), "no auto-accept under the default policy");
  console.log("  ✓ default approval policy = manual gate (unchanged; conformance-safe)");
}
...
async function trustAutoAboveThresholdAutoAccepts() {
  ...
  assert.ok(!types.includes("approval.requested"), "no manual gate once auto-accepted");
  const acc = events.find((e) => e.type === "approval.accepted");
  assert.equal(acc.data.auto, true, "the auto-accept is flagged (transparent), and carries a proofpack");
  assert.ok(acc.data.proofpack, "auto-accept still writes a ProofPack — the record stays complete");
  console.log("  ✓ 信任后自动 above threshold auto-accepts with full approval.accepted stream + ProofPack");
}

async function main() {
  console.log("e2e-approval-policy: SETTINGS approval policy is honored end to end");
