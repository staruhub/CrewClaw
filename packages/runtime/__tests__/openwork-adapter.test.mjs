...
import openworkAdapter, { openworkAdapter as namedOpenworkAdapter } from "../adapters/openwork.mjs";
...
await check("compile returns blueprint with exactly the 11.3 nine keys", () => {
  assert.deepEqual(Object.keys(blueprint).sort(), expectedBlueprintKeys.toSorted());
});

await check("tool_bindings includes abstract web-search without vendor names", () => {
  const serialized = JSON.stringify(blueprint.tool_bindings);
  assert.match(serialized, /web-search/);
  assert.doesNotMatch(serialized, /tavily|playwright|bash|curl|selenium|puppeteer|serper|openai|anthropic/i);
});

await check("permission rules keep P0 auto-allow and P4 deny-safe", () => {
  const rules = blueprint.permission_gateway_rules;
  assert.equal(rules.tiers.P0.auto_allow, true, "P0 read-public must auto-allow");
  assert.equal(rules.tiers.P0.approval_required, false, "P0 read-public must not require approval");
  assert.ok(
    rules.tiers.P4.auto_allow === false || rules.tiers.P4.default_action === "deny",
    "P4 must not be auto-allowed"
  );
  assert.notEqual(rules.tiers.P4.auto_allow, true, "P4 auto_allow must never be true");
});
...
await check("runSmokeTest is dry-run Seed 2.1 eval trial", () => {
  ...
  assert.match(`${smoke.id} ${smoke.task} ${smoke.seed} ${smoke.name}`, /Seed 2\.1|seed-2\.1/i);
});
