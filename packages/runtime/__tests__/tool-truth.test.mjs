...
assert.equal(findState(emptyStates, "utility.weather")?.status, STATUS.unavailable);
assert.equal(findState(emptyStates, "artifact.write")?.status, STATUS.available);

assert.equal(findState(emptyStates, "memory.write", "session")?.status, STATUS.available);
assert.equal(findState(emptyStates, "memory.write", "persistent")?.status, STATUS.unavailable);
...
assert.equal(findState(tavilyStates, "utility.weather")?.status, STATUS.unavailable);
