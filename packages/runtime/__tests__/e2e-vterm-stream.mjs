...
function renderInline(s) {
  ...
  s = s.replace(/\[([^\]]+)\]\(([^)]+)\)/g, "\x1b[4;36m$1\x1b[24;39m\x1b[2m($2)\x1b[22m");
...
function blankReappearFrames(history) {
  ...
  for (const frame of history) {
    frame.snapshot.forEach((line, row) => {
      ...
      const prev = rowState.get(row) || { phase: "start", before: null, blank: null };
      ...
      if (text) {
        ...
        rowState.set(row, { phase: "blanked", before: prev.before, blank: frame });
      }
    });
  }
...
const out = {
  isTTY: true,
  columns: 80,
  write(s) { return term.write(s, current); },
};
const md = createMdPrinter(true, { out, isTTY: true, renderMdLine, renderTable, isTableRow, visibleLen, GUTTER });
...
console.log(`e2e-vterm-stream passed: ${term.history.length} frames, zero disappear frames`);
