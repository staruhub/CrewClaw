...
for (const tok of [
  ...
  run.sink.onDelta(tok);

// every snapshot's answer must extend (or equal) the previous — never shrink or blank
for (let i = 1; i < answers.length; i++) {
  assert.ok(
    answers[i].startsWith(answers[i - 1]),
    `answer must grow monotonically, never blank: "${answers[i - 1]}" → "${answers[i]}"`
  );
}
assert.equal(
  answers[answers.length - 1],
  "这是一段会逐字流出的中文回答",
  "the full streamed text is preserved"
);
