...
import { clean, dedupe, formatResults, pickBackend, cleanHtml } from "../tools-web.mjs";
...
assert.deepEqual(deduped.map((r) => r.title), ["A", "B"]);
...
assert.equal(pickBackend({ SERPER_API_KEY: "k", BRAVE_API_KEY: "k" }).name, "serper");
assert.equal(pickBackend({ TAVILY_API_KEY: "k" }).name, "tavily");
assert.equal(pickBackend({ TAVILY_API_KEY: "k", SERPER_API_KEY: "k" }).name, "tavily"); // tavily wins
assert.equal(pickBackend({}).name, "ddg");

// formatResults: numbered list with urls + a follow-up-with-fetch hint
const text = formatResults("北京天气", [
  { title: "Weather", url: "https://wttr.in/beijing", snippet: "Sunny 21C", age: "1h" },
...
assert.ok(md && /Title Here/.test(md) && /Hello/.test(md), `cleanHtml should yield markdown text, got: ${md}`);
