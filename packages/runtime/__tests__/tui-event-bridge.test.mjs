...
let changes = 0;
const run = createTaskRun({ employee: { name: "鲸" }, mode: "Chat" }, () => changes++);

run.start("查杭州天气", "Chat");
run.sink.onDelta("计划：");
run.sink.onDelta("查数据源。");
run.sink.onInvocation({ toolName: "web_fetch", action: "读取 wttr.in", line: "🌐 wttr.in (412 字)", status: "success" });
...
assert.ok(toolLine && toolLine.status === "✓", "tool invocation became a ✓ timeline line carrying the result summary");
assert.ok(s.timeline.some((l) => l.status === "✓" && l.label.includes("完成")), "completion line");
assert.ok(changes >= 6, "onChange fired per event");

// a blocked tool → ✗ with the structured code
const r2 = createTaskRun();
r2.start("x");
r2.sink.onInvocation({ toolName: "web_search", action: "已跳过", status: "blocked", code: "blocked_serp" });
