...
import {
  ...
  linkCount,
  routeBySize,
} from "../web-extract.mjs";

assert.equal(linkCount("see [a](x) and [b](y)"), 2);
assert.ok(
  hasSpaMarker(
    '<html><div id="root"></div><script>__NEXT_DATA__={}</script></html>'
  )
);
assert.equal(
  hasSpaMarker("a real article with lots of words and no markers"),
  false
);
assert.ok(
  isJsShell({
    markdown: "[Home](/) [Docs](/d) [Login](/l)",
    html: '<div id="app"></div>',
  })
);
assert.equal(
  isJsShell({ markdown: "X".repeat(600), html: '<div id="app"></div>' }),
  false
);
