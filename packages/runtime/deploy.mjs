...
export function deployToOpenWork(pkg, { root = process.cwd() } = {}) {
  ...
  const compatibility = computeCompatibility(pkg, openworkAdapter.capabilities());
  ...
  return { ok: true, dir, blueprintPath, level: compatibility.level, targetLevel, reasons: compatibility.reasons || [] };
}
...
if (process.argv[1] && fileURLToPath(import.meta.url) === path.resolve(process.argv[1])) {
  ...
  if (!agent) { console.error("Usage: node deploy.mjs <agent>"); process.exit(2); }
  const pkgPath = path.resolve(root, "experts", agent, "crewclaw.employee.yaml");
  if (!existsSync(pkgPath)) { console.error(`Error: 找不到员工包：${pkgPath}`); process.exit(1); }
  const loaded = loadEmployeePackage(pkgPath); // { ok, package, errors } — package is the pkg
  if (!loaded.ok) { console.error("员工包校验失败：\n  - " + (loaded.errors || []).join("\n  - ")); process.exit(1); }
  const r = deployToOpenWork(loaded.package, { root });
  if (!r.ok) { console.error("部署校验失败：\n  - " + r.errors.join("\n  - ")); process.exit(1); }
  console.log(`✓ 已生成 OpenWork 部署包 → ${r.dir}`);
  console.log(`  目标兼容等级：${r.level}（目标 ${r.targetLevel}）`);
  if (r.reasons.length) console.log("  说明：\n  - " + r.reasons.join("\n  - "));
}
