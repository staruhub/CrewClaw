...
export const SPEC_VECTORS = Object.freeze([
  ...
  },
  {
    ...
    input: '最新有哪些模型发布',
    expect: { route: 'employee_task', needsSearch: true, blockedWhenNoKey: true },
  },
  {
    id: 4,
...
export const HOST_SPEC = Object.freeze([
  { id: 'CC-SCOPE-001', desc: 'Employees must declare job boundaries and delegation scope.' },
  ...
  { id: 'CC-EVID-001', desc: 'Research conclusions must bind to Evidence Cards.' },
  { id: 'CC-ACT-001', desc: 'Bare numeric input must match PendingAction before model inference.' },
  { id: 'CC-SAFE-001', desc: 'High-risk actions require explicit confirmation.' },
...
const HOST_ONLY_EXPECTATIONS = Object.freeze({
  upgrade: 'TaskRun creation and report/table artifact production require host orchestration.',
  requiresArtifactObject: 'Artifact object creation and artifact.created emission require host orchestration.',
  noBareBash: 'Reveal/open-folder dispatch must be checked at the workspace or OS adapter boundary.',
  requiresRender: 'requires_render is emitted by the web extraction/render pipeline, not this pure module harness.',
  entersApprovalNotDone: 'Approval state is decided by the TaskRun outcome state machine.',
  matchesPendingActionFirst: 'PendingAction precedence is a host dispatch contract before model inference.',
});
...
export async function runSpecVector(vector, ctx = {}) {
  ...
  if ('needsSearch' in expect || 'blockedWhenNoKey' in expect) {
    ...
    if ('blockedWhenNoKey' in expect) {
      addCheck(
        checks,
        'blockedWhenNoKey',
        (searchState?.status === STATUS.missing_key) === expect.blockedWhenNoKey,
  ...
  if ('noFalsePersistentClaim' in expect) {
    ...
    const falseClaim = truth.persistent !== 'available' && /长期|持久|persistent/i.test(response.note);
