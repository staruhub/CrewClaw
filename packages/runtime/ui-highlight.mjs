...
const NUMBER_RE = /^(?:0[xX][0-9a-fA-F_]+|0[bB][01_]+|0[oO][0-7_]+|\d[\d_]*(?:\.\d[\d_]*)?(?:[eE][+-]?\d[\d_]*)?)/;

const COMMON_LITERALS = new Set(["true", "false", "null", "undefined", "None", "nil", "NULL", "TRUE", "FALSE"]);
...
const SPECS = {
  ...
  bash: {
    ...
    strings: ["'", "\"", "`"],
    keywords: [
      "alias", "bg", "break", "case", "cd", "command", "continue", "do", "done", "elif", "else", "esac", "eval",
      "exec", "exit", "export", "fg", "fi", "for", "function", "if", "in", "local", "printf", "read", "return",
      "select", "set", "shift", "source", "then", "time", "trap", "until", "while",
    ],
  },
  rust: {
    line: ["//"],
  ...
  html: {
    ...
    strings: ["'", "\""],
    keywords: [
      "a", "article", "body", "button", "div", "em", "footer", "form", "h1", "h2", "h3", "head", "header", "html",
      "img", "input", "label", "li", "link", "main", "meta", "nav", "ol", "option", "p", "script", "section",
      "select", "span", "strong", "style", "table", "tbody", "td", "textarea", "th", "thead", "title", "tr", "ul",
      "xml",
    ],
  },
  css: {
    ...
    strings: ["'", "\""],
    keywords: [
      "align-items", "animation", "background", "border", "color", "display", "flex", "font", "gap", "grid", "height",
      "margin", "none", "padding", "position", "relative", "absolute", "static", "sticky", "width",
    ],
  },
  sql: {
    line: ["--"],
    ...
    strings: ["'", "\""],
    keywords: [
      "ALTER", "AND", "AS", "ASC", "BY", "CREATE", "DELETE", "DESC", "DISTINCT", "DROP", "FROM", "GROUP", "HAVING",
      "IN", "INSERT", "INTO", "IS", "JOIN", "LEFT", "LIKE", "LIMIT", "NOT", "NULL", "ON", "OR", "ORDER", "RIGHT",
...
function specFor(lang) {
  ...
  return {
    ...spec,
    keywordSet: new Set((spec.keywords || []).map((w) => (spec.caseInsensitive ? w.toLowerCase() : w))),
    literalSet: new Set([...(spec.literals || []), ...COMMON_LITERALS].map((w) => (spec.caseInsensitive ? w.toLowerCase() : w))),
  };
}
