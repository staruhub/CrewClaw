...
const MEMORY_ITEMS: [MemoryItem; 6] = [
    ...
    MemoryItem {
        ...
        title: "客户偏好双交付格式",
        date: "07-06",
        body: "客户偏好 .md + .csv 双交付。\n→ 报告类任务默认两种格式都产出。",
    },
    MemoryItem {
        ty: "P",
        title: "playbook: model-research v3",
        date: "07-05",
        body: "调研五步：定范围 → 抓数据 → 交叉验证 → 出结论 → 附来源。\n抽检率 100%，来源必须官方页面。",
    },
    MemoryItem {
        ty: "E",
        title: "预算类任务默认三档方案",
        date: "07-04",
        body: "预算类任务默认给三档方案（省/稳/快）。\n→ 已在 3 次任务中验证有效，客户满意度提升。",
    },
    MemoryItem {
        ty: "K",
        title: "无搜索 key 环境预检",
        date: "07-03",
        body: "无搜索 key 的环境要提前预检工具可用性。\n→ 任务开始前先跑 tool.check。",
    },
    MemoryItem {
        ty: "P",
        title: "playbook: budget-triage v1",
        date: "07-02",
        body: "预算任务三步：识别约束 → 生成选项 → 排序推荐。",
    },
];
...
fn render_memory_panel(frame: &mut Frame<'_>, ui_state: &UiState, area: Rect) {
    ...
    let detail_lines: Vec<Line> = detail_body
        ...
        .collect();
    frame.render_widget(
        Paragraph::new(Text::from(detail_lines)).wrap(Wrap { trim: true }),
        rows[3],
    );
}
