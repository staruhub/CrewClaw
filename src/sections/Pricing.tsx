              ...
              >
                ...
                <div className="p-7 md:p-8 flex-1 flex flex-col">
                  <div className="flex items-start justify-between gap-4 border-b border-white/8 pb-6">
                    <div>
                      <h3 className="font-mono text-lg font-semibold text-crew-heading">{plan.name}</h3>
                      <p className="mt-2 text-xs uppercase tracking-[0.2em] text-white/40">{plan.desc}</p>
                    </div>
                    {plan.featured && (
                  ...
                  <div className="mt-7">
                    <span className="font-sans text-[40px] font-light text-crew-heading">{plan.price}</span>
                  </div>

                  <ul className="mt-7 space-y-3 flex-1">
                    {plan.features.map((f) => (
                      <li key={f} className="flex items-start gap-3 border-b border-white/6 pb-3 last:border-b-0 last:pb-0">
                        <Check
                          size={14}
                          className="mt-0.5 shrink-0"
                          style={{ color: plan.featured ? "#C87941" : plan.accent }}
                        />
                  ...
                  <div className="mt-6">
                    <button
                      onClick={plan.featured ? () => onGetStarted("pro") : plan.name === "Enterprise" ? onContact : () => onGetStarted(plan.name.toLowerCase())}
