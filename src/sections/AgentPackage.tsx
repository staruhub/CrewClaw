...
const layers = [
  {
    ...
    file: "SOUL.md",
    layer: "Soul",
    desc: "Role, workflow, boundaries, escalation rules",
  },
  {
    color: "#D4853A",
    file: "skills/**/SKILL.md",
    layer: "Skills",
    desc: "Reusable Hermes procedures for repeated work",
  },
  {
    color: "#E8A87C",
    file: "mcp.json",
    layer: "Tools",
    desc: "Optional external tools with explicit permissions",
  },
  {
    color: "#8B6F4E",
    file: "CERTIFICATION.md",
    layer: "Trust",
    desc: "ChaoGeek tuning notes, tests, and known limits",
  },
  {
    color: "#A89B91",
    file: "distribution.yaml",
    layer: "Manifest",
    desc: "Hermes install metadata and owned files",
  },
];
            ...
            return (
              <ScrollReveal key={expert.name} delay={index * 0.06}>
                <article className="flex h-full flex-col rounded-[8px] border border-white/10 bg-white/[0.018] p-5 transition-colors hover:border-crew-copper/30 md:p-6">
                  ...
                  <p className="mt-4 flex-1 text-sm leading-7 text-crew-body">
                    {expert.description}
                  </p>
                  ...
                  <div className="mt-5 rounded-[8px] border border-white/8 bg-black/18 p-3">
                    ...
                    <code className="mt-2 block break-words font-mono text-[12px] leading-6 text-white/70">
                      {installCommand ??
                        (isAvailable
                          ? "Public package pending · use the source checkout"
                          : "Join waitlist for launch notice")}
                    </code>
                  </div>

                  {installCommand ? (
                    <button
                      onClick={() => copyInstall(expert.name, installCommand)}
          ...
          <div className="polished-panel">
            ...
            {layers.map((item, i) => (
              ...
              >
                ...
                <span className="font-mono text-[11px] uppercase tracking-[0.22em] text-white/48">
                  {item.layer}
                </span>
                <span className="text-sm leading-relaxed text-crew-body">
                  {item.desc}
                </span>
              </div>
            ))}
