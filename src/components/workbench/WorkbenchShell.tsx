...
export function WorkbenchShell({ run }: { run: TaskRun }) {
  ...
  const selectedArtifact = useMemo(
    () => run.artifacts.find((artifact) => artifact.id === selectedArtifactId) ?? run.artifacts[0] ?? null,
      ...
      <section className="mx-auto max-w-7xl">
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-white/10 pb-5">
          <div className="min-w-0">
            ...
            <h1 className="mt-4 text-2xl font-semibold leading-tight md:text-3xl">{run.user_goal}</h1>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Badge className={cn("rounded-[8px] border font-mono", statusClass(run.status))} variant="outline">
            ...
            {typeof run.cost === "number" ? (
              <Badge className="rounded-[8px] border border-white/10 bg-white/[0.03] font-mono text-crew-body" variant="outline">
                ${run.cost.toFixed(2)}
                {typeof run.tokens === "number" ? ` · ${run.tokens.toLocaleString()} tok` : ""}
              </Badge>
            ) : null}
          </div>
        </div>
        ...
        <div className="mt-5 flex flex-wrap gap-2 border-b border-white/10 pb-5">
          {run.pending_actions.map((action) => (
            <button
              key={action.key}
              type="button"
              onClick={() => setLastAction(`${action.key} ${action.command ?? action.label}`)}
            ...
            >
              <span className="font-mono text-crew-copper">[{action.key}]</span> {action.label}
            </button>
          ))}
          {lastAction ? (
            <span className="self-center font-mono text-xs text-crew-muted">pending.run {lastAction}</span>
          ) : null}
        </div>
        ...
        <div className="mt-6 grid gap-5 lg:grid-cols-[1.2fr_0.9fr]">
          ...
          <div className="space-y-5">
            <ArtifactPanel artifacts={run.artifacts} selectedId={selectedArtifact?.id ?? null} onSelect={setSelectedArtifactId} />
