...
export function ArtifactPreview({ artifact }: { artifact: WorkbenchArtifact | null }) {
  if (!artifact) {
    return (
    ...
    <section className="border border-white/10 bg-white/[0.025] p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="font-mono text-xs uppercase tracking-[0.18em] text-crew-muted">Preview</p>
          <h2 className="mt-1 truncate text-base font-semibold text-crew-heading">{artifact.name}</h2>
          {artifact.path ? <p className="mt-1 break-all font-mono text-xs text-crew-muted">{artifact.path}</p> : null}
        </div>
