...
export function SectionLabel({
  ...
  return (
    <ScrollReveal className={`flex flex-col ${alignClass} ${className}`}>
      <span
        className={`mb-4 font-mono text-[10px] uppercase tracking-[0.28em] md:text-[11px] ${textColor}`}
      >
        {`( ${label} )`}
      </span>
      <h2
        className={`font-sans text-[34px] font-light leading-[1.04] md:text-[52px] ${titleColor}`}
      >
      ...
      {description && (
        <p
          className={`mt-4 max-w-[560px] text-sm leading-7 md:text-[15px] ${descColor}`}
        >
          {description}
        </p>
      )}
