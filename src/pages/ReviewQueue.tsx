...
function RiskBadge({ submission }: { submission: CreatorSubmission }) {
  if (submission.high_risk_permissions.length === 0) {
    return (
      <Badge className="rounded-[8px] border-emerald-400/25 bg-emerald-400/10 text-emerald-200" variant="outline">
        Standard
      </Badge>
    );
  }
  ...
  return (
    <Badge className="gap-1 rounded-[8px] border-amber-300/35 bg-amber-300/10 text-amber-100" variant="outline">
    ...
    <div className="space-y-5">
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="rounded-[8px] border border-white/10 bg-white/[0.03] p-4">
          ...
          <p className="mt-1 text-sm text-crew-muted">{submission.manifest.role}</p>
        </div>
        <div className="rounded-[8px] border border-white/10 bg-white/[0.03] p-4">
          <p className="font-mono text-xs uppercase tracking-[0.16em] text-crew-muted">
            Creator
          ...
          <p className="mt-1 text-sm text-crew-muted">v{submission.manifest.version}</p>
        </div>
      </div>

      <div className="rounded-[8px] border border-white/10 bg-white/[0.03] p-4">
        <h3 className="text-sm font-medium text-crew-heading">Manifest summary</h3>
      ...
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <SummaryList label="Onboarding" items={submission.manifest.install_requirements} />
        <SummaryList label="Limitations" items={submission.manifest.limitations} />
        <SummaryList label="Input examples" items={submission.manifest.input_examples} />
      </div>
      <div>
        <h3 className="mb-3 text-sm font-medium text-crew-heading">Permissions</h3>
        <PermissionLevelList compact permissions={submission.manifest.permissions} />
      </div>
    </div>
  );
}
    ...
    <main className="min-h-screen bg-crew-bg px-4 py-10 text-crew-heading sm:px-6">
      <section className="mx-auto max-w-7xl">
        <div className="flex flex-col gap-5 border-b border-white/10 pb-8 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-base leading-7 text-crew-body">
              Review manifest quality, flag risky permissions, and only publish employees
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Button asChild className="rounded-[8px] bg-crew-copper text-white hover:bg-crew-bronze">
        ...
        <section className="mt-8 grid gap-4 md:grid-cols-3">
          <Metric icon={AlertTriangle} label="Pending" value={pending.length.toString()} />
          <Metric icon={ShieldAlert} label="High risk" value={highRiskCount.toString()} />
        ...
        <Alert className="mt-6 rounded-[8px] border-crew-copper/25 bg-crew-copper/10 text-crew-heading">
          ...
          <AlertDescription className="text-crew-body">
            Mail sending, contacts write, payment, and delete permissions must be reviewed
            before an employee receives Verified Employee status.
          </AlertDescription>
        </Alert>
          ...
          <CardContent className="p-0">
            {rows.length === 0 ? (
              <div className="px-6 pb-8">
                <h2 className="text-2xl font-light">No employees waiting for review.</h2>
                <p className="mt-3 max-w-xl text-sm leading-6 text-crew-body">
                  Submitted employees from Creator Console will appear here before they can
                  become public Verified Employees.
                </p>
              </div>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow className="border-white/10 hover:bg-transparent">
                    ...
                    <TableHead className="px-5 text-crew-muted">Manifest summary</TableHead>
                    <TableHead className="px-5 text-crew-muted">Risk</TableHead>
                    <TableHead className="px-5 text-crew-muted">Status</TableHead>
                    <TableHead className="px-5 text-crew-muted">Submitted</TableHead>
                    <TableHead className="px-5 text-right text-crew-muted">Actions</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {rows.map((submission) => (
                    ...
                    >
                      <TableCell className="px-5 py-5">
                        ...
                        <div className="mt-1 text-xs text-crew-muted">
                          {submission.manifest.role} · {submission.manifest.creator}
                        </div>
                      </TableCell>
                      ...
                      <TableCell className="px-5 py-5">
                        <div className="flex flex-wrap justify-end gap-2">
                          <Dialog>
                            <DialogTrigger asChild>
                              <Button className="rounded-[8px] border-white/15" size="sm" variant="outline">
                            ...
                            <DialogContent className="max-h-[86vh] overflow-auto rounded-[8px] border-white/10 bg-[#17120F] text-crew-heading sm:max-w-4xl">
                              <DialogHeader>
                                <DialogTitle>{submission.manifest.name}</DialogTitle>
                                <DialogDescription className="text-crew-body">
                          ...
                          {submission.status === "submitted" ? (
                            <>
                              <Button
                                ...
                                onClick={() => approve(submission.submission_id)}
                                size="sm"
                              >
                                <CheckCircle2 className="size-4" />
                                Approve
                              </Button>
                              <RejectDialog onReject={reject} submission={submission} />
                            </>
                          ) : null}

                          {submission.status === "published" && !submission.disabled ? (
                            <AlertDialog>
                              ...
                              <AlertDialogContent className="rounded-[8px] border-white/10 bg-[#17120F] text-crew-heading">
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription className="leading-6 text-crew-body">
                                    This employee will be taken off the public bench and will
                                    lose Verified Employee status.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                  ...
                                  <AlertDialogAction
                                    ...
                                    onClick={() => disable(submission.submission_id)}
                                  >
                                    Disable employee
                                  </AlertDialogAction>
                                </AlertDialogFooter>
