...
- Prepare the v0.20 product milestone for reproducible GitHub publication through the Beta 0.7 channel.
...
- Harden Windows path canonicalization and local-only mutation boundaries in the runtime and CLI.
- Add production-safe Fly.io binding and verify the deployed workflow at `https://crewhire.fly.dev/`.
- Add regression and browser coverage for the design-aligned closed loop.

Employee-package-specific changes remain documented in each package's `CHANGELOG.md`.
